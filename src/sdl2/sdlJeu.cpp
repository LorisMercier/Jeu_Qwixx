...
sdlJeu::sdlJeu () : jeu() {
    ...
    this->widthScreen = 900;
    this->hightScreen = dm.h-50;

    // Creation de la fenetre
    window = SDL_CreateWindow("Projet AnLoVi : Qwixx", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 900, hightScreen, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (window == NULL) {
    ...
    //Texte Reco
    txtbtnRec.setSurface(TTF_RenderText_Solid(fontstandard,"Recommencer",couleurPolice));
    txtbtnRec.loadFromCurrentSurface(renderer);
...
void sdlJeu::sdlDes(int x,int y) const{
    int c = hightScreen/10;
    tabDeBlanc[jeu.des.getBlanc1()-1].draw(renderer,x,y,c,c);
    tabDeBlanc[jeu.des.getBlanc2()-1].draw(renderer,x+120,y,c,c);

    tabDeRouge[jeu.des.getRouge()-1].draw(renderer,x+280,y,c,c);
    tabDeJaune[jeu.des.getJaune()-1].draw(renderer,x+400,y,c,c);
    tabDeVert[jeu.des.getVert()-1].draw(renderer,x+520,y,c,c);
    tabDeBleu[jeu.des.getBleu()-1].draw(renderer,x+640,y,c,c);
...
void sdlJeu::sdlFicheActive(const SDL_Rect &posFiche, int numJoueur){
    ...
    tabFontPseudo[numJoueur].draw(renderer,400,posFiche.y+360,100,30); 
...
void sdlJeu::sdlAff(const SDL_Rect &posFiche, const SDL_Rect &boutonFin, int numJoueur, int joueurPrincipal, bool blanc){
    ...
    deroulement.draw(renderer,10,posFiche.y+130,155,30); 
    tourBlanc.draw(renderer,10,posFiche.y+170,145,30);
    tourCouleur.draw(renderer,10,posFiche.y+210,175,30);

    //Affichage Joueur Actif
    joueurActif.draw(renderer,10,posFiche.y,170,30);
    tabFontPseudoRouge[joueurPrincipal].draw(renderer,10,posFiche.y+40,90,27); 

    //Bouton Fin de partie
    SDL_SetRenderDrawColor(renderer, 50, 209, 40, 255);
    SDL_RenderFillRect(renderer,&boutonFin);

    txtbtnFin.draw(renderer,795,posFiche.y+310,50,30);

}

void sdlJeu::sdlAffFin(const SDL_Rect &boutonRec){
    SDL_SetRenderDrawColor(renderer,200,200,200,255);
    SDL_RenderClear(renderer);

    //Affichage image de fond
    fond.draw(renderer,0,0,900,hightScreen);    
    ...
    SDL_SetRenderDrawColor(renderer, 50, 209, 40, 255);
    SDL_RenderFillRect(renderer,&boutonRec);

    txtbtnRec.draw(renderer,410,370,120,30);
...
void sdlJeu::sdlBoucle () {
    ...
    SDL_Event events;
    SDL_Rect posFiche = {200,400,500,350};  
    SDL_Rect boutonFin = {770,700,100,50}; 
    SDL_Rect boutonRecommencer = {400,360,140,50};   
	bool quit = false;
	...
	while (!quit) {   
		...
		while (SDL_PollEvent(&events)) {
			if (events.type == SDL_QUIT) quit = true;           // Si l'utilisateur a clique sur la croix de fermeture
                ...
                if(events.type == SDL_MOUSEBUTTONDOWN && events.button.button == SDL_BUTTON_LEFT && etat == 2){
                    if(cliqueDansRect(events.motion.x,events.motion.y,boutonRecommencer)){
                        jeu.init();
                        etat = 0;
                    }
                }
